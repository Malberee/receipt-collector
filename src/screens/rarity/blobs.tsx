...
export const Blobs: FC<BlobsProps> = ({ color }) => {
  ...
  const topPathStart = Skia.Path.MakeFromSVGString(
  ...
  const topPathEnd = Skia.Path.MakeFromSVGString(
  ...
  const topPath = usePathInterpolation(
    progress,
    [0, 1],
    [topPathStart, topPathEnd],
  )

  const bottomPathStart = Skia.Path.MakeFromSVGString(
  ...
  const bottomPathEnd = Skia.Path.MakeFromSVGString(
  ...
  const bottomPath = usePathInterpolation(
    progress,
    [0, 1],
    [bottomPathStart, bottomPathEnd],
  )
  ...
  return (
    <>
      ...
      >
        <Path path={bottomPath} color={color} />
      </Canvas>
    </>
  )
}
